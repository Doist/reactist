...
<Subtitle>A controlled, composable, and accessible sidebar</Subtitle>

The `Sidebar` component is a resizable panel that supports being docked on the left or right, or rendered as an overlay. Its uses
include a docked left nav, a resizable conversation list, a mobile drawer, or a floating side pane. It's fully controlled, so the
consumer owns the breakpoints and the state of the panel.

Here it is as a docked navigation panel whose collapse toggle stays reachable
while the panel is slid away:

<Canvas of={SidebarStories.CollapsibleNav} />

A docked sidebar is an in-flow flex child that holds its width. Toggling `isOpen`
collapses it with a `margin-inline` transition while the main content reflows into
the freed space. A `width` is required for the collapse: it derives the collapse
margin, so without one the panel never slides out, and the component logs a dev
warning.
...
`<Sidebar>`'s own props are in the Playground controls near the end of this page;
the panel and its slots follow here. Each also forwards `data-*` and native element
attributes to its rendered node.
...
The sidebar sizes itself; the surrounding flex context, its controlled state, and
its visual skin are the consumer's. The shell items live on the parent because a
child cannot set them on its parent or siblings:
