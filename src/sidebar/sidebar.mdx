...
Do not pair it with `unmountOnHide` on `<Sidebar>`: that unmounts the whole panel (this control included) at the end of the close transition, so nothing is left to stay operable while closed. The two contradict each other, and the component warns when both are set.
