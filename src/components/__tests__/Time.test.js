...
const { unix } = TimeUtils

describe('Time', () => {
    const testDate = unix(new Date('March 22, 1991 13:37:42'))

    it('renders without crashing', () => {
        const time = shallow(<Time time={Date.now()} />)
    ...
    it('does not render short absolute time on hover when tooltipOnHover is set', () => {
        const time = shallow(
            <Time time={Date.now()} tooltipOnHover expandOnHover />
        )
        time.simulate('mouseEnter', getMouseEvent())
    ...
    it('does not render full absolute time on hover when tooltipOnHover is set', () => {
        const time = shallow(
            <Time time={Date.now()} tooltipOnHover expandFullyOnHover />
        )
        time.simulate('mouseEnter', getMouseEvent())
        expect(time.find('Tooltip').props().children).toBe('moments ago')
    })
