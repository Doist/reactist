...
A checkbox field with a `ReactNode` as label (in this example the `ReactNode` contains an icon and a string).
...
<Notice tone="info">
    If you want to render an icon, please pass in the icon via the appropriate icon attribute
    instead.
</Notice>
