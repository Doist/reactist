...
The **Modal** component consists of four subcomponents *Box, Header, Body, Actions* which are exported as keys of the Modal object. Hence, to use a Modal your code should look like this:
```
...
export const WithPlaygroundTemplate = ({ data, ...args }) => (
   <section>
            <div id="modal_box" />
...
export const ModalTemplate = ({ body = "", title, subtitle = "", buttonTitle, buttonText, medium = false, plain = false }) => {
    const modal = (
        <Modal.Box medium={medium} closeOnOverlayClick>
            <Modal.Header title={title} subtitle={subtitle} />
            <Modal.Body plain={plain}><ReactMarkdown children={body} /></Modal.Body>
        </Modal.Box>
    )
...
<Preview withToolbar>
  <Story
    parameters={{ docs: { source: { type: "code"} }}} 
    name="ModalHeaderBodyAndActionsStory">
